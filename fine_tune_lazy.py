#!/usr/bin/env python3
"""
Modified version of fine_tune.py with lazy Unsloth import to avoid GPU detection issues.
"""
...
# Delay Unsloth import - will be imported in functions that need it
# from unsloth import FastLanguageModel  # MOVED TO FUNCTION
...
def setup_model_and_tokenizer(model_name: str, model_family: str, config: Dict[str, Any]):
    ...
    # Import Unsloth here (lazy import)
    try:
        from unsloth import FastLanguageModel
    except Exception as e:
        logger.error(f"Failed to import Unsloth: {e}")
        logger.error("This might be due to GPU state issues. Try running: sudo ./reset_gpu.sh")
        raise
...
def test_generation(model_path: str, model_family: str):
    ...
    try:
        ...
        logger.error(f"Failed to import Unsloth for testing: {e}")
        return
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Fine-tune various models on Manim dataset (with lazy Unsloth import)",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Use full model name
  python fine_tune_lazy.py --model "Qwen/Qwen2.5-Coder-7B-Instruct"
  
  # Use shorthand
  python fine_tune_lazy.py --model qwen-7b
  
  # Use model from models.txt (if it contains 'qwen2.5-coder:7b')
  python fine_tune_lazy.py --model qwen2.5-coder:7b
  
  # List supported models
  python fine_tune_lazy.py --list-models
        """
    )
