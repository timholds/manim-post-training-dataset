#!/usr/bin/env python3
"""
Fixed version of fine_tune.py that avoids GPU corruption.
Key changes:
1. Removed save_pretrained_merged() which causes GPU corruption
2. Added proper GPU cleanup
...
def train_model(model_name: str):
    ...
    try:
        ...
        # Save final model - ONLY LoRA adapters, not merged
        logger.info("Saving LoRA adapters...")
        model.save_pretrained(str(output_dir))
        tokenizer.save_pretrained(str(output_dir))
        
        # IMPORTANT: We skip save_pretrained_merged() to avoid GPU corruption
        logger.info("Note: Skipping merged model save to prevent GPU corruption.")
        logger.info("To use the model, load the LoRA adapters with the base model.")
        ...
        # Clean up model and trainer references
        del model
        del trainer
        del tokenizer
        gc.collect()
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Fine-tune various models on Manim dataset (FIXED VERSION)",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
This is the FIXED version that avoids GPU corruption by:
1. Not using save_pretrained_merged()
2. Properly cleaning up GPU resources
3. Better error handling

Examples:
  # Use full model name
  python fine_tune_fixed.py --model "Qwen/Qwen2.5-Coder-7B-Instruct"
  
  # Use shorthand
  python fine_tune_fixed.py --model qwen-7b
  
  # List supported models
  python fine_tune_fixed.py --list-models
        """
    )
    ...
    def cleanup():
        """Cleanup function to restart Ollama and clean GPU on exit."""
        cleanup_gpu_resources()
        if not args.keep_ollama:
